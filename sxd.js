...
var sxd = {
    ...
    init: function() {
        ...
        this.files.init('sxdGrid2', {
            header: [
                ...
                [this.lng('dt'), 90],
                [this.lng('type'), 30],
                [this.lng('tab'), 30, 1, this.lng('table')],
                [this.lng('records'), 60, 1],
                [this.lng('size'), 45, 1],
                [this.lng('comment')]
            ],
            width: 565,
    ...
    timer: {
        set: function(time1, time2, proc) {
            ...
            if (!time1 && !time2) this.t1.innerHTML = '00:00', this.t2.innerHTML = '00:00';
            else {
                var t = time2 - time1;
    ...
    informer: function() {
        ...
        sxd.job.timer = setTimeout(function() {
            sxd.ajax('info', sxd.lng('run'), {
                job: sxd.job.name,
                act: 'info',
                lseek: sxd.job.log_seek
            }, 1, function() {
                sxd.informer();
            });
        }, 250);
    ...
    runServices: function(type) {
        switch (type) {
            case 'delete':
                ...
                sxd.ajax('index', sxd.lng('run'), {
                    act: 'delete_db',
                    name: this.combos.services_db.value
                });
                return;
        }
        this.result.clear();
    ...
    addCombo: function(name, value, ico, opts) {
        ...
        if (typeof opts == 'object') {
            ...
            var tmp = {};
            if (this.combos[t[2]].options.length > 0) tmp[opts] = this.combos[t[2]].options[this.combos[t[2]].sel].opts;
            else tmp[opts] = [];
    ...
    clearOpt: function(name) {
        if (name) {
            ...
            for (var o in this.options) this.options[o].length = 0;
    },
    showMenu: function(obj, opts, cfg) {
        ...
        for (var o in opts) {
            ...
            s.onmousedown = function() {
                if (cfg.runjob) sxd.runSavedJob(m.obj.com, this.value);
                else if (cfg.btn) sxd.runServices(this.value);
                else m.obj.select(this.value, 0);
    ...
    offset: function(el) {
        var top = el.offsetTop + el.offsetHeight + 1,
    ...
    keyNatSort: function(a, b) {
        ...
        while ((aa = /^(\D+|(\d+))(.*?)$/.exec(a)) && (bb = /^(\D+|(\d+))(.*?)$/.exec(b))) {
            if (aa[2] && bb[2] && aa[2] != bb[2]) return aa[2] * 1 > bb[2] * 1 ? 1 : -1;
            else if (aa[1] && bb[1] && aa[1] != bb[1]) return aa[1] > bb[1] ? 1 : -1;
            else if (aa[3] && bb[3] && aa[3] != bb[3]) a = aa[3], b = bb[3];
            else return 0;
        }
    },
    formatSize: function(size, add) {
        ...
        if (size >= 0) {
            ...
            if (add == 2) return size;
            else if (add == 1) return ' <span>[ ' + size + ' ]</span>';
            return '[ ' + size + ' ]';
        }
        return '';
    },
    ...
    return {
        ...
        drawTree: function(d) {
            ...
            for (var k = 0, l = d.length; k < l; k++) {
                ...
                index[o.id] = o;
                if (o.pid > 0) index[o.pid].childs.push(o);
                else data.push(o);
            }
            this.lastId = 0;
        ...
        drawChilds: function(aChilds, pid) {
            ...
            var sTree = '<ul>';
            var cLi, cPlus, cCheck, cIcon, sChilds, o, nChild = 0,
                lastChild = aChilds.length,
                pCheck = -1,
                chSize = 0;
                    ...
                    for (var i2 = 0, l2 = o.childs.length; i2 < l2; i2++) {
                        ...
                        if (c.checked == 4) {
                            for (var i3 = 0, l3 = c.childs.length; i3 < l3; i3++) {
                                tc = c.childs[i3];
                                if (tc.checked == 1) saved['TA'].push(tc.name);
                                else if (tc.checked == 2) saved['TC'].push(tc.name);
                            }
                        } else if (c.checked == 1) saved[c.type.substr(1, 2)].push(c.name);
                        else if (c.checked == 2) saved['TC'].push(c.name);
                    }
                } else if (o.checked == 1) saved[o.type.substr(1, 2)].push('*');
                else if (o.checked == 2) saved['TC'].push('*');
            }
            return saved;
        },
        error: function(str) {
            ...
            this.el.onmousedown = function(e) {
                ...
                if (_this.getId(e || window.event)) {
                    ...
                    if (_this.curBtn.more) {
                        ...
                        _this.curBtn.pop = 1;
                        sxd.showMenu(_this.curBtn, sxd.options['sj_' + _this.curBtn.com], {
                            width: 220,
                            runjob: true
                        });
